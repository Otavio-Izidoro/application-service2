package com.estudo.applicationservice.constants;

public class PresenceLogs {

    public static final String UPDATE_ERROR_PRESENCE = "Erro ao atualizar presenca para o dia ";

    public static final String SUCESSFULLY_UPDATED = "Presença Atualizada para o dia ";

    public static final String NON_EXISTENT_PRESENCE = "Atualize a presença para o dia ";

    public static final String UPDATED_CONTENT = "Conteúdo Atualizado para o dia ";

    public static final String PRESENCE_NOT_FOUND = "Presença não encontrada!";

    public static final String TOPIC_NOT_FOUND = "Nenhum topico registrado!";
}
